...
import { existsSync, readFileSync, writeFileSync, readdirSync, statSync, appendFileSync, mkdirSync, rmSync } from "node:fs";
...
const PanelsSettingsSchema = z.object({
  ...
  leftOpen: z.boolean().default(true),
    ...
    webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          const text = truncated ? raw.subarray(0, MAX_READ).toString("utf8") : raw.toString("utf8");
    ...
    webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          if (!target || !existsSync(target)) return send(res, 400, { ok: false, error: "path required or not found" });
    ...
    webServer.register({
      ...
      handler: async (req, res) => {
        try {
          if (req.method !== "POST") return send(res, 405, { ok: false, error: "method not allowed" });
          const body = await readBody(req);
          const dir = body?.dir ? resolve(body.dir) : process.cwd();
          const query = String(body?.query || "").toLowerCase();
          const mode = body?.mode || "both";
          if (!query || !existsSync(dir) || !isDirectory(dir)) {
          ...
          function walk(d, depth) {
            ...
            for (const e of entries) {
              ...
              let line = 0, snippet = "";
              if (mode !== "name" && (nameMatch || mode === "content")) {
    ...
    webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          const target = resolve(url.searchParams.get("path") || "");
          if (!target || !existsSync(target)) return send(res, 400, { ok: false, error: "path required" });
    ...
    webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          if (!dir || !existsSync(dir) || !isDirectory(dir)) return send(res, 400, { ok: false, error: "path required" });
      ...
      webServer.register({
        ...
        handler: async (req, res) => {
          try {
            ...
            const session = { proc, buffer: "", exited: false };
            proc.onData((data) => { session.buffer += data; });
      ...
      webServer.register({
        ...
        handler: (req, res) => {
          try {
            ...
            const out = since < session.buffer.length ? session.buffer.slice(since) : "";
            send(res, 200, { ok: true, out, at: session.buffer.length });
          } catch (e) {
            send(res, 500, { ok: false, error: String(e?.message ?? e) });
          }
        }
